...
Application.prototype.onload = function() {
        ...
        function onmidimessage(event) {
            ...
            switch(status) {
              ...
              case 0xb0:
                // for mosaic button
                // Change Voice Number
                var ccmsg=msg[1], ccval=msg[2];
                if ( ccmsg== 0x2b && ccval==0x7f) {
                    fireEvent("button_progUp");
                }
                if (ccmsg == 0x2c && ccval==0x7f) {
                    fireEvent("button_progDown");
                }
                if (ccmsg == 0x2a && ccval==0x7f) {
                    fireEvent("button_velsens");
                }
                if (ccmsg == 0x29 && ccval==0x7f) {
                    fireEvent("button_egloop");
                }
                // Update knob_fine
                //if (ccmsg == 0x00) {
                if (ccmsg == 0x18) {
                    ...
                    document.getElementById("knob_velocity").style.setProperty("top", vmax-(1-val)*(vmax-vmin)+"px");
                }
                ...
                if (ccmsg == 0x15) {
                    ...
                    document.getElementById("knob_coarse").style.setProperty("top", vmax-val*(vmax-vmin)+"px");
                }
                // Update knob_fine
                //if (ccmsg == 0x03) {
                if (ccmsg == 0x16) {
                    ...
                    document.getElementById("knob_fine").style.setProperty("top", vmax-(1-val)*(vmax-vmin)+"px");
                }
                // Update feedback
                //if (ccmsg == 0x04) {
                if (ccmsg == 0x17) {
    ...
    // send WebMidiLink's "ready" event when synth is ready
    LinkReady();
};
